...
fn endpoints_from_interfaces(interfaces: Vec<Interface>) -> Vec<ProbeEndpoint> {
    ...
    for interface in interfaces {
        ...
        let endpoint = match interface.addr {
            IfAddr::V4(address) => address.broadcast.map(|broadcast| ProbeEndpoint {
            ...
            IfAddr::V6(address) => ipv6_endpoint(&interface.name, interface.index, address.ip),
        };
...
mod tests {
    ...
    fn builds_scoped_link_local_ipv6_probe() {
        let endpoint = ipv6_endpoint("enp1s0", Some(7), "fe80::1234".parse().unwrap())
    ...
    fn builds_scoped_site_local_ipv6_probe() {
        let endpoint = ipv6_endpoint("enp1s0", Some(9), "fd12:3456::1".parse().unwrap())
    ...
    fn rejects_ipv6_probe_without_scope() {
        assert_eq!(
            ipv6_endpoint("enp1s0", None, "fe80::1234".parse().unwrap()),
            None
        );
    }
