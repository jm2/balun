...
# ordinary runtime codecs remain outside Balun's app-owned boundary.
# This initial port assumes a trusted local build artifact; hostile-bundle
# resource budgets and stable input snapshot/reopen checks remain release work.
...
[ "$ref_count" -eq 1 ] || {
    echo "Flatpak bundle must contain exactly one Balun app ref (found $ref_count)" >&2
    exit 1
}

ostree --repo="$repository" checkout "$app_ref" "$checkout"
[ -d "$checkout/files" ] && [ ! -L "$checkout/files" ] || {
    echo "Flatpak app commit does not contain a files payload" >&2
    exit 1
}
[ -f "$checkout/metadata" ] && [ ! -L "$checkout/metadata" ] || {
