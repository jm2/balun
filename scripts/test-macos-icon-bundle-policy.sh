...
TEST_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/balun-macos-icon-policy.XXXXXX")"
...
assert_no_decode_directories() {
  local temporary
  for temporary in "$POLICY_TMPDIR"/balun-macos-icons.*; do
    [[ ! -e "$temporary" ]] \
...
ICONSET="${TEST_ROOT}/balun.iconset"
...
APP_BUNDLE="${TEST_ROOT}/Balun.app"
PLIST="${APP_BUNDLE}/Contents/Info.plist"
RESOURCES="${APP_BUNDLE}/Contents/Resources"
mkdir -p "$RESOURCES"
printf '<plist/>\n' > "$PLIST"
printf 'balun\n' > "${PLIST}.icon-name"
printf 'valid-icns\n' > "${RESOURCES}/balun.icns"
...
mv "${RESOURCES}/balun.icns" "${RESOURCES}/saved.icns"
assert_status 1 macos_validate_app_icon_bundle "$APP_BUNDLE" "$BUNDLE_ID"
assert_reason_contains 'does not resolve'
mv "${RESOURCES}/saved.icns" "${RESOURCES}/balun.icns"
